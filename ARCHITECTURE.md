...
```text
Tin nhắn
  ├─ trùng cache 10 tin ─> parser + luật hiện tại ─> hiển thị (0 lượt AI)
  └─ tin mới
...
Mỗi yêu cầu Gemini thật chiếm một trong 12 lượt của phiên và được ghi vào `sessionStorage` gồm thời điểm, vai trò, model, độ dài đầu vào và kết quả tóm tắt. Lỗi 429/5xx/timeout chuyển model tối đa hai lần với trễ 300/600 ms; lỗi mạng, khoá và quyền dừng ngay. Cache và lịch sử nằm trong `localStorage`, tối đa 10 tin.
...
GitHub Pages chỉ phục vụ tệp tĩnh. API key dùng trực tiếp trong trình duyệt có thể bị người dùng xem; bản trình diễn phải giới hạn referrer/quota. Bản sản xuất cần proxy/serverless giữ khoá ở máy chủ. Việc giải redirect chắc chắn cho mọi shortener cũng cần proxy có chống SSRF và danh sách đích cho phép.
